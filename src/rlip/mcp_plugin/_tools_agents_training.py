...
Tools: rl_train_agent, rl_train_agent_auto, rl_get_training_result
"""
...
def rl_train_agent(
    ...
    **For most use-cases, prefer** ``rl_train_agent_auto()`` **instead.**
    ``rl_train_agent_auto`` handles language setup, baseline training,
...
# ── Automated full-pipeline training ─────────────────────────────────────────


def rl_train_agent_auto(
    agent_type: str,
    env_id: str,
    instruction: str = "",
    ...
    """
    Fully automated agent training pipeline — one call, all stages run
    automatically in the background.

    The pipeline always follows this fixed sequence:

    1. **Language translation check** — validates that a translator is
       registered for *env_id*.  Fails early if none is found.
    2. **Baseline training** — trains a fresh agent on raw environment
       observations.  A LanguageTrackingWrapper records which
       language-described states are visited during successful episodes.
    3. **Instruction derivation** — after baseline training, the top-k states
       most correlated with success are scored and ranked.  Skipped when
       *instruction* is provided.
    4. **Instruction matching** — the top derived instruction (or the
       user-supplied *instruction*) is matched to the best language state
       in the populated observation cache; no new exploration is required.
    5. **Instruction-shaped training** — a fresh agent is trained on the
       same environment with sequential sub-goal reward shaping applied to
       the matched instruction stages.
    ...
    env_id:
        A registered RLIP environment ID, e.g. ``"Sailing-v0"``.
    instruction:
        Optional natural-language instruction.  When provided, stages 3–4
        use this instruction directly instead of auto-deriving one.
    n_episodes_baseline:
        Number of training episodes for the baseline phase.
    n_episodes_instruction:
        Number of training episodes for the instruction-shaped phase.
    max_steps:
        Step cap per episode (both phases).
    seed:
        Optional reproducibility seed.
    top_k:
        Maximum instruction candidates derived from baseline (ignored when
        *instruction* is provided).  The top-scoring candidate is used.
    min_episode_visits:
    ...
    alpha / gamma / epsilon / epsilon_min / epsilon_decay:
        tabular_q hyper-parameters (also epsilon params for dqn).
    hidden_size / lr:
    ...
    derived instruction used, and clean evaluation metrics.
    """
    from ..environments.registry import registry as _env_registry  # noqa: PLC0415
    from ..language_translation import get_translator  # noqa: PLC0415
    ...
    if translator is None:
        return (
            ...
            "A translator is required for the automated pipeline.\n"
            "Register one with rl_set_translator_code() first."
        )
    ...
    def _job() -> None:
        ...
        class _TrackBar:
            """Thin env shim: tqdm progress bar + LanguageTrackingWrapper."""
            def __init__(self, w: Any, n: int) -> None:
                self._w = w
            ...
            def step(self, action: Any) -> Any:
                ...
                try:
                    self._ep_r += float(r.reward if hasattr(r, "reward") else r.get("reward", 0.0))
                except Exception:
                    pass
                return r
            def close(self) -> None:
            ...
            @property
            def env_id(self) -> str:
                return self._w.env_id
            def __getattr__(self, name: str) -> Any:
                return getattr(self._w, name)

        track_env = _TrackBar(wrapper, n_episodes_baseline)
        _training_jobs[job_id]["progress_env"] = track_env
        _training_jobs[job_id]["phase"] = "baseline"

        agent1 = _build_agent(agent_type, alpha, gamma, epsilon, epsilon_min, epsilon_decay, hidden_size, lr, seed)
        try:
            result1 = agent1.train(track_env, n_episodes=n_episodes_baseline, max_steps=max_steps, seed=seed)
        except Exception as exc:
            _training_jobs[job_id]["status"] = "failed"
        ...
        # ── Phase 2a: Determine instruction to use ────────────────────────────
        ...
        if instruction:
            ...
            log.info("rl_train_agent_auto: derived instruction — %s", derived[0].instruction)

        # ── Phase 2b: Match instructions to observation states ────────────────
        ...
        stage_languages: list[list[str]] = [
            ...
            for m in protocol.matches
        ]

        # ── Phase 2c: Instruction-shaped training ─────────────────────────────
        _training_jobs[job_id]["phase"] = "instruction training"
        _training_jobs[job_id]["n_episodes"] = n_episodes_baseline + n_episodes_instruction

        shaped_env = _SequentialShapedEnv(
            env2,
            stage_languages=stage_languages,
            bonus=None,   # auto-scale
            threshold=sub_goal_threshold,
            translator=_translator,
            env_id=env_id,
        )

        agent2 = _build_agent(agent_type, alpha, gamma, epsilon, epsilon_min, epsilon_decay, hidden_size, lr, seed)
        progress_env2 = _ProgressEnv(
            shaped_env,
            n_episodes=n_episodes_instruction,
            agent_type=agent_type,
            env_id=env_id,
        )
        _training_jobs[job_id]["progress_env"] = progress_env2

        try:
            result2 = agent2.train(progress_env2, n_episodes=n_episodes_instruction, max_steps=max_steps, seed=seed)
        except Exception as exc:
            _training_jobs[job_id]["status"] = "failed"
            _training_jobs[job_id]["result"] = (
                f"Instruction-shaped training failed: {exc}\n\n"
                f"Baseline agent saved as agent_id='{baseline_agent_id}'."
            )
            try:
                progress_env2.close()
        ...
        # Store instruction-shaped agent
        instr_agent_id = uuid.uuid4().hex[:12]
        _trained_agents[instr_agent_id] = {
            "agent": agent2,
            "env_id": env_id,
            "agent_type": agent_type,
            "train_result": result2,
            "use_language_state": False,
            "best_episode_history": getattr(result2, "best_episode_history", []),
            "instructions_used": instructions_to_use,
            "instruction": instructions_to_use[0] if instructions_to_use else "",
        }

        # ── Clean evaluation of instruction-shaped agent ───────────────────────
        _eval_mean: Optional[float] = None
        _eval_std: Optional[float] = None
        try:
            _eval_mean, _eval_std, _ = _run_clean_evaluation(
                agent=agent2,
                env_factory=factory,
                n_episodes=100,
                max_steps=max_steps,
            ...
            _trained_agents[instr_agent_id]["eval_mean"] = _eval_mean
            _trained_agents[instr_agent_id]["eval_std"] = _eval_std
        except Exception:
            pass
        ...
        result_text = (
            ...
            f"compare_agent_ids=['{baseline_agent_id}']) to compare both phases."
        )
    ...
    instr_note = f"  Instruction:   {instruction!r}\n" if instruction else "  Instruction:   (auto-derived from baseline)\n"
    return (
        f"Automated training pipeline started — {agent_type} on {env_id}\n\n"
        f"  Job ID:        {job_id}\n"
        f"{instr_note}"
        ...
        f"  2. {'Matching provided instruction' if instruction else 'Deriving instructions from baseline'}\n"
        f"  3. Instruction-shaped training\n\n"
        f"Call rl_get_training_result(job_id='{job_id}') to check progress and get the result."
    )
