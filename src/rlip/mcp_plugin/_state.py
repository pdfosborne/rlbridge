...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "2. rl_train_agent_auto(agent_type, env_id) – DEFAULT tool for training. "
        "Automatically runs the full pipeline: baseline training with language "
        "tracking → instruction derivation → instruction-shaped training.  "
        "Pass instruction= to specify a goal instead of auto-deriving one.  "
        "Returns a job_id immediately (no timeout).\n"
        "   • Use rl_train_agent() only when you need manual control (no translator, "
        "custom match_id, or explicit hyper-parameter tuning without auto-pipeline).\n"
        ...
        "IMPORTANT — training is asynchronous: rl_train_agent_auto(), rl_train_agent(), and "
        "rl_train_and_derive_instructions() all return immediately with a job_id. "
        ...
        "PREFERRED: rl_train_agent_auto(agent_type, env_id, instruction=instruction) – "
        "one call handles the full pipeline (language setup → baseline → matching → "
        "shaped training).  Use this unless you need to run stages separately.\n"
        "MANUAL ALTERNATIVE (run stages separately):\n"
        ...
        "PREFERRED: rl_train_agent_auto(agent_type, env_id) – no instruction= needed; "
        "automatically derives one from baseline training and runs the full pipeline.\n"
        "MANUAL ALTERNATIVE (run stages separately):\n"
