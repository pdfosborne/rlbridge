...
def obs_cache_langs(env_id: str) -> list[str]:
    ...
    :func:`match_instruction` or :func:`build_instruction_following_protocol`
...
# ── Global in-memory plan DB registry ────────────────────────────────────────
...
def build_instruction_following_protocol(
    ...
    *,
    ...
    record_history: bool = True,
    extra_sub_goals: Optional[list[str]] = None,
) -> InstructionFollowingProtocol:
    """
    ...
    sub_goal_bonus:
        ...
        When *None* (default) the bonus is auto-scaled via
        :func:`scale_sub_goal_bonus`: ``max_reward / (100 × n_sub_goals)``
        where *n_sub_goals* is the number of matched language states
        (``len(InstructionMatch.matched_states)``) and *max_reward* is
        inferred from the environment's reward range (falls back to 1.0).
        This keeps the shaping signal two orders of magnitude below the
        terminal goal reward and scales down further when many sub-goals
    ...
    extra_sub_goals:
        Optional list of additional natural-language sub-step instructions
        derived from decomposing *instruction*.  Each string is matched
        against the observation corpus (uses the cache — no re-exploration)
        and its best-matching language states are appended as extra co-equal
        sub-goals alongside the primary match.  This is how LLM-decomposed
        sub-steps feed into the reward-shaping protocol.
    ...
    # Resolve env_id the same way match_instruction does.
    env_id = getattr(env, "env_id", type(env).__name__)

    # Retrieve the cache entry populated by match_instruction so the protocol
    # can record episode success back into it.
    cache_entry = _INSTRUCTION_CACHE.get(env_id, {}).get(instruction)

    # Live reference to the state→instructions map so newly matched
    # instructions are visible to the protocol without recreation.
    state_instr_map = _STATE_INSTRUCTIONS.setdefault(env_id, {})
    ...
    # Match each LLM-derived sub-step and add its best states as extra sub-goals.
    # match_instruction() re-uses the obs cache so no new exploration is needed.
    if extra_sub_goals:
        primary_langs = {match.matched_language} | set(extra_langs)
        for sub_instruction in extra_sub_goals:
            try:
                sub_match = match_instruction(
                    sub_instruction,
                    env,
                    encoder=encoder,
                    translator=translator,
                ...
                for lg, _obs, _sc in sub_match.matched_states:
                    if lg not in primary_langs:
                        extra_langs.append(lg)
                        primary_langs.add(lg)
            except Exception:  # noqa: BLE001 – sub-step matching is best-effort
                pass

    # Auto-scale the bonus when not explicitly provided.
    # n_sub_goals = primary + extras so the total shaped reward budget is
    # spread evenly across every co-equal sub-goal state.
    n_sub_goals = 1 + len(extra_langs)
    effective_bonus: float = (
        sub_goal_bonus
        if sub_goal_bonus is not None
        else scale_sub_goal_bonus(env, n_instructions=n_sub_goals)
    )
...
def derive_instructions_from_training(
    ...
    *,
    ...
    :func:`build_instruction_following_protocol`.
...
def train_and_derive_instructions(
    ...
    **train_kwargs:
    ...
    ::
        ...
        protocol = build_instruction_following_protocol(best_instr, env)
        result = protocol(env)
    """
