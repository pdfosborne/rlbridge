...
OpenCode  (~/.config/opencode/config.json):
    ...
    # or manually:
    {
      "mcp": {
        "rlbridge": {
          "type": "local",
          "command": ["python", "-m", "rlbridge.mcp_plugin"]
        }
      }
    }

If installed via pip as a console script, replace ``["python", "-m",
"rlbridge.mcp_plugin"]`` with ``"rlbridge-mcp"`` (use ``--use-script`` flag).
...
def _register_environment_custom_tools() -> int:
    ...
    ``rlbridge.environment_mcp_tools`` entry-point group.
    """
    total_registered = 0
    try:
...
def main() -> None:
    """Entry-point called by the ``rlbridge-mcp`` console script."""
