...
- ``rlbridge.environments`` - register environment factories
- ``rlbridge.environment_mcp_tools`` - register environment-specific MCP tools
"""
...
_ENV_ENTRY_GROUP = "rlbridge.environments"
_MCP_ENTRY_GROUP = "rlbridge.environment_mcp_tools"


def load_plugin_environments(registry: Any = None) -> int:
    """Load and register all installed ``rlbridge.environments`` entry points."""
    if registry is None:
...
def load_plugin_mcp_tools(
    ...
    **kwargs: Any,
    ...
    """Load and register all installed ``rlbridge.environment_mcp_tools`` entry points."""
    total = 0
